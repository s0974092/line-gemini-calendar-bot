...
`;
  ...
  afterAll(() => {
    Date.prototype.getFullYear = originalGetFullYear; // Restore original Date.prototype.getFullYear
  });
  ...
  test('should correctly parse CSV with descriptive shifts for a specific person (怡芳)', () => {
    ...
    // Check a few more specific events for '怡芳'
  ...
  test('should correctly parse CSV with time-based shifts (CJ)', () => {
    const personName = 'CJ';
    const events = parseCsvToEvents(baseCsvContent, personName);
...
測試員,全職,早班,,休`;
    ...
    expect(events.length).toBe(1); // Only '早班' should be parsed
    expect(events[0]).toEqual(expect.objectContaining({
      title: '測試員 早班',
